...
git init // git clone <repository_url> [<directory_name>]
...
git branch -M main
git push -u origin main（<远程仓库> <本地分支>:<远程分支>）
```
...
- 查看全部配置：`git config --list和git config --global --list`
- 查看远程仓库：`git remote -v`
- 查看标签：`git tag`

- 假设远程仓库把默认分支从main改为develop：`git remote set-head origin -a  # 更新HEAD引用`
  # 现在：
  `git log -1 remotes/origin/HEAD --oneline`
  # 输出：d4e5f6a 新功能 （develop分支的最新提交）
  `git log -1 remotes/origin/main --oneline`
  # 输出：a1b2c3d 修复bug （main分支保持不变）
...
- 删除特定的 safe.directory 配置：`git config --global --unset safe.directory "E:/StudyDevelopment-Journey/Git"``git config --global --unset safe.directory`
...
- 删除已有的 origin：`git remote remove origin`
...
- 先同步再合并/变基：`git fetch origin（<远程分支>:<本地分支>）` 后 `git merge origin/<分支>` 或 `git rebase origin/<分支>`
...
- 重命名分支为您想要的名字：`git branch -M my-custom-branch-name`
- 配置全局默认分支名：`git config --global init.defaultBranch my-custom-branch-name`
- 处理换行符：`git config --global core.autocrlf true（win）``（Linux/macOS）git config --global core.autocrlf input``git config --global core.autocrlf false（禁用)`
